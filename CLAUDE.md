...
## [자동 처리 원칙 — 매우 중요]
- **사용자에게 작업을 요청하기 전에 반드시 먼저 "MCP 도구 / CLI / 코드로 자동 처리할 수 있는가?"를 검토한다.**
- **자동으로 가능한 작업은 사용자에게 시키지 않고 즉시 직접 처리한다.** 사용자가 모바일에서 작업하는 경우가 많으므로 한 단계라도 줄이는 것이 우선이다.
- **사용자 작업 안내 전 체크리스트**:
  1. Cloudflare 정보 조회·설정 확인 → `mcp__93329c11-...` (accounts/workers/D1/R2/KV/Hyperdrive) 도구 먼저 시도
  2. Supabase 스키마·로그·쿼리·마이그레이션 → `mcp__a1ff7b2c-...` 도구 먼저 시도
  3. GitHub PR/이슈/파일/코드 검색 → `mcp__github__*` 도구 먼저 시도
  4. Sentry 에러·이슈 분석 → `mcp__927b60bf-...` 도구 먼저 시도
  5. 코드 수정·빌드·테스트 → Bash/Edit/Write로 직접 처리
- **사용자에게 요청해도 되는 작업은 다음뿐이다**:
  - 새 API Token / Secret Key 발급 (보안상 사람만 가능)
  - GitHub Secrets·환경변수 등록 (쓰기 권한 부여 안 됨)
  - 비밀번호·인증코드 입력
  - 외부 서비스 가입 / 결제 / 충전
  - 물리적 작업 (단말기 연결, 네트워크 케이블 등)
- **위 5가지 외의 작업을 사용자에게 시키기 전에 한 번 더 자문한다**: "내가 직접 할 수 있는데 시키는 건 아닌가?"
- **이미 시켜놓은 작업이라도 자동화 가능했음을 깨달으면 즉시 사용자에게 사과하고 자동 처리로 전환한다.**
